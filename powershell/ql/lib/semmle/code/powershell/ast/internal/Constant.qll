...
class ConstBoolean extends ConstantValue, TConstBoolean {
  ...
  final override VarAccess getAnExpr() {
    ...
    toRaw(result).(Raw::VarAccess).getUserPath() = "true"
    or
    this.asBoolean() = false and
    toRaw(result).(Raw::VarAccess).getUserPath() = "false"
  }
}
...
class NullConst extends ConstantValue, TNull {
  ...
  final override VarAccess getAnExpr() { toRaw(result).(Raw::VarAccess).getUserPath() = "null" }
}
