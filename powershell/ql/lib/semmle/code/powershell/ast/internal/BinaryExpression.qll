...
class BinaryExpr extends Expr, TBinaryExpr {
  /** INTERNAL: Do not use. */
  int getKind() { result = toRaw(this).(Raw::BinaryExpr).getKind() }

  Expr getLeft() {
    synthChild(this, binaryExprLeft(), result)
    or
    not synthChild(this, binaryExprLeft(), _) and
    toRaw(result) = toRaw(this).(Raw::BinaryExpr).getLeft()
  }

  Expr getRight() {
    synthChild(this, binaryExprRight(), result)
    or
    not synthChild(this, binaryExprRight(), _) and
    toRaw(result) = toRaw(this).(Raw::BinaryExpr).getRight()
  }
